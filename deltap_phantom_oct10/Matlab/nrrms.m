function [diff, rms,rms2,noise,sigt]=nrrms(epath, scannum ,mpath)
...
stand=std(reshape(imcrop2(mag(:,:,2:59),roi2),1,numel(imcrop2(mag(:,:,2:59),roi2))));
%[std,~]=meanroi(imcrop2(mean(mag(:,:,2:2),3),roi2));
hold off
imagesc(imcrop2(mean(mag(:,:,2:4),3),roi2))
%imagesc(mean(mag(:,:,2:4),3))
...
sigtinv=(2*3.14*alpha*gamma*b*te*abs(dat2))/(sqrt(2)*sigmaa);
sigt=(sigtinv).^-1;

%sigtinv=sigtinv.^2;
if scannum==12;
   diff=diff(52:112,93:123,:);
   sigtinv=sigtinv(52:112,93:123,:);
   %sigtinv=sigtinv(52:112,93:153,:);
   etmap=etmap(52:112,93:123,:); 
   %etmap=etmap(52:112,93:153,:);
elseif scannum==22;
   diff=diff(46:106,98:128,:);
   %diff=diff(46:106,98:158,:);
   sigtinv=sigtinv(46:106,98:128,:);
   %sigtinv=sigtinv(46:106,98:158,:);
   etmap=etmap(46:106,98:128,:);
   %etmap=etmap(46:106,98:158,:);
end

noise=squeeze(mean(mean(mean(sigtinv(:,:,:),2),1))).^-1
...
%montage0(sigtinv.^-1);
figure;
rms2=ones(1,numel(sigtinv(1,1,:)));
for index=1:numel(sigtinv(1,1,:));
    rms2(index)=sqrt(sum(sum((diff(:,:,index).^2).*sigtinv(:,:,index),2),1)/sum(sum(sigtinv(:,:,index),1),2));
end
plot(squeeze(rms2(2:numel(rms2))));
ylabel('RMS difference (^oC)');
xlabel('Time(s)');

% [~,means]=meanroi(imcrop2(mean(sigtinv(:,:,2:4),3),roi));
% means
