...
%xtmap=readVTK2(xpath,numel(etmap(1,1,:)));
load(mpath);
mtmap=Model;
...
imnum=find(squeeze(mtmap(ii,jj,:))==tmax)-2;%not at max time t because of errors associated with laser shutoff time
...
clf
plot(xaxis,mtmap(ii,:,imnum),'r','LineWidth',2)
hold;
plot(xaxis,etmap(ii,:,imnum),'bo','MarkerSize',6)
plot(xaxis(jj),mtmap(ii,jj,imnum),'kd','MarkerSize',12,'LineWidth',2);
%title('Axial Profile')
ylim([0,15]);
xlim([-15,15]);
xlabel('Distance (mm)')
...
figure
plot(yaxis,mtmap(:,jj,imnum),'r','LineWidth',2)
hold;
plot(yaxis,etmap(:,jj,imnum),'bo','MarkerSize',6)
plot(yaxis(ii),mtmap(ii,jj,imnum),'kd','MarkerSize',12,'LineWidth',2);
...
figure
plot(taxis,mtemp,'r','LineWidth',2)
hold;
plot(taxis,etemp,'bo','MarkerSize',6)
plot(taxis(imnum),mtmap(ii,jj,imnum),'kd','MarkerSize',12,'LineWidth',2)
