...
function [total, dice, hd] = temperature_obj_fxn_GPU_cond ( inputdatavars, sources, mu_eff_list, k_cond_list );
...
cond_length = length(k_cond_list);
...
[tmap_unique] = Human_GPU_cond ( power_log,spacing,scaling,mod_point,source,w_perf,k_cond_list,g_anisotropy,mu_eff_list,probe_u,robin_co,c_blood);
...
total = zeros(cond_length,6);
total(:,1) = k_cond_list;
% mu_eff   L2     weird metric     model-base   max(model)

% isotherms = 51:65
dice = zeros(cond_length,15);
hd = dice;
