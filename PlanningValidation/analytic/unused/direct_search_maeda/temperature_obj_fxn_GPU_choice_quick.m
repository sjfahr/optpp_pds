...
function [total, dice, hd, mutual_threshold, false_pix] = temperature_obj_fxn_GPU_choice_quick ( inputdatavars, sources, mu_eff_list, w_perf, k_cond, choice );
...
% isotherms = 51:65
dice = zeros(n_length,1);
hd = dice;
mutual_threshold = dice;
false_pix = zeros(n_length,3);
...
for ii = 1:n_length
    ...
    model_deg_threshold = tmap_model_scaled_to_MRTI(:,:,ii) >= 57;
    ...
    dice(ii) = 2 * n_intersection / ( n_model + n_MRTI );  % DSC
    mutual_threshold(ii) = mi( model_deg_threshold, MRTI_deg_threshold); % MI for label map
    false_pix (ii,1) = n_MRTI - n_intersection;  % False negative
    false_pix (ii,2) = n_model - n_intersection; % False positive
    false_pix(ii,3) = false_pix(ii,1)+false_pix(ii,2); % total false pixels
    if isempty(mod_list)==1
        hd(ii) = 1;
    else
        hd(ii) = HausdorffDist( mod_list, MRTI_list);
    end
