...
elseif choice ==6
    ...
    for ii = 1:n_length
        ...
        for kk=1:15
            ...
            %MRTI_deg_threshold(:,:,kk) = dose;
            dose22 = permute (dose, [2, 1, 3]);
            MRTI_deg_threshold(:,:,kk) = dose22(:,:,end);
            %MRTI_deg_threshold(:,:,kk) = imfill( ExtractNLargestBlobs(MRTI_deg_threshold(:,:,kk),1) , 'holes')  ;  % 1st, it keeps the largest contiguous region, then it fills in any hole
    ...
    MRTI_crop = dose22(:,:,end);
    
    %intersection = model_deg_threshold + 2.*model_deg_threshold;
