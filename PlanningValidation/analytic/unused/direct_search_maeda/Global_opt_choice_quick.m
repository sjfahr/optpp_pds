% This function finds the best mu_eff for the different studies.

function Global_opt_choice_quick (choice);
...
for ii = 2:(num_studies+1)
    ...
    [total{ii,9}(1) , index] = min (total{ii,4}); % record optimal Hausdorff Distance
    ...
    [total{ii,10}(2,1) , index] = max (total{ii,5});  % MI for 57 C isotherm
    ...
    [total{ii,11}(1) , index] = min (total{ii,6}(:,3));  % False pixel number for 57 C isotherm
    total{ii,11}(2) = total{ii,2}(index,1); % record value that produces number of false pixels
    total{ii,11}(3) = index;
...
if choice == 1
   ...
   save ('GPU_global_mu_quick.mat','total','summary');
    
elseif choice == 2
    
    save ('GPU_global_perf_quick.mat','total','summary');
    
elseif choice == 3
    
    save ('GPU_global_cond_quick.mat','total','summary');
