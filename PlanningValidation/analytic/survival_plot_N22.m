%
function [] = survival_plot_N22 (global_SS_dice, naive_values, dice_opt,naive_tag,FEM_dice,gradient_SS_dice);

%close all

sizes.var = size(global_SS_dice,1);
sizes.dice = size(global_SS_dice,2);

thresholds = linspace ( 0.0, 1, 10001);
pass_global_SS_dice = cell(sizes.var,sizes.dice);
naive_pass = pass_global_SS_dice;
L2_mean_pass =pass_global_SS_dice;
L2_median_pass = pass_global_SS_dice;
dice_mean_pass = pass_global_SS_dice;
dice_median_pass = pass_global_SS_dice;
for ii = 1:sizes.var
    
    for jj = 1:sizes.dice
        pass_global_SS_dice{ii,jj} = zeros(10001,1);
        naive_pass{ii,jj} = pass_global_SS_dice{ii,jj};
...
for kk = 1:10001
    ...
    passes_LOOCV (kk) = sum(global_SS_dice > thresholds(kk) );
